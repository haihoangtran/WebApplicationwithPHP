...
class FormValidator {
    ...
    function validate_uniqueLogin($input_value, &$default_error_message) {
        $bret = true;
        $query = "SELECT m_login FROM Members WHERE m_login='$input_value'";
    ...
    function ValidateCommand($command, $command_value, $input_value, &$default_error_message, $variable_name, $formvariables) {
        ...
        switch ($command) {
            ...
            case 'ulogin': {
                    $bret = $this->validate_uniqueLogin($input_value, $default_error_message);
                    break;
                }
            case 'maxlen': {
